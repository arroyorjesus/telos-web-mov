    ...
    <section className="relative py-14 overflow-hidden">
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid md:grid-cols-3 gap-5">
          ...
          >
            <p className="text-xs uppercase tracking-widest mb-5" style={{ color: 'rgba(255,255,255,0.35)' }}>Contacto directo</p>
            <div className="grid sm:grid-cols-3 gap-4">
              {INFO.map((item) => (
                <div key={item.label} className="flex flex-col gap-2">
                  ...
                  {item.href ? (
                    <a href={item.href} className="text-sm font-medium leading-snug hover:text-telos-green transition-colors" style={{ color: 'rgba(255,255,255,0.85)' }}>
                      {item.value}
                    </a>
                  ) : (
                    <p className="text-sm leading-snug" style={{ color: 'rgba(255,255,255,0.6)' }}>{item.value}</p>
                  )}
                </div>
              ))}
            </div>
            ...
            <div className="mt-5 pt-5" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
              <p className="text-xs mb-3" style={{ color: 'rgba(255,255,255,0.35)' }}>Horario de atención</p>
              <div className="grid sm:grid-cols-3 gap-2">
                {SCHEDULE.map((s) => (
                  <div key={s.days} className="rounded-xl p-3" style={{ background: 'rgba(255,255,255,0.06)' }}>
                    <p className="text-xs" style={{ color: 'rgba(255,255,255,0.4)' }}>{s.days}</p>
                    <p className="text-sm font-medium mt-0.5 text-white">{s.hours}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          >
            <div>
              ...
              <h3 className="font-bold text-white mb-2">WhatsApp directo</h3>
